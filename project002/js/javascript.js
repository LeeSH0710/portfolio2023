$(document).ready(function () {
  ...
  // 첫번째em을 클릭시 첫번째em은 사라지고 두번째em을 나타나게하고 nav의 class에 on을 추가해라
  $("header em:nth-child(1)").click(function () {
    $("header em:nth-child(1)").fadeOut(500);
    $("header em:nth-child(2)").fadeIn(2000);
    $("header nav").fadeIn().addClass("on");
  });

  // 두번째em을 클릭시 두번째em은 사라지고 첫번째em을 나타나게하고 nav의 class에 on을 제거해라
  $("header em:nth-child(2)").click(function () {
    $("header em:nth-child(1)").fadeIn(500)
    $("header em:nth-child(2)").fadeOut(1000)
    $("header nav").fadeOut().removeClass("on");
  });

  //   nav li 메뉴 클릭시 각 스크롤위치로 이동
  ...
  // 닌텐도 소개부분의 사진이 자동으로 왼쪽에서 오른쪽으로 넘어간다.
  ...
  $("#wrap section:nth-child(4) article").mouseover(function () {
    $("#wrap section:nth-child(4) article img").addClass("on").fadeIn(800);
  });

  $("#wrap section:nth-child(4) article").mouseout(function () {
    $("#wrap section:nth-child(4) article img").removeClass("on");
  });
